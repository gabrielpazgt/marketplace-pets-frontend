...
export class ProductDetailComponent implements OnInit, OnDestroy {
  ...
  get subcategoryLabel(): string {
    return (this.product?.subcategory || '').trim();
  }
  ...
  get detailFacts(): ProductFact[] {
    ...
    if (ingredientNames.length) {
      facts.push({
        ...
        note: ingredientNames.length > 3 ? `y ${ingredientNames.length - 3} mas vinculados en catalogo.` : 'Relaciones listas para enriquecer la ficha.',
      });
    }
  ...
  get productSpecs(): ProductSpec[] {
    ...
    pushSpec('Categoria', this.resolveCategoryLabel(product.category));
    pushSpec('Formato', this.resolveFormLabel(product.form));
    pushSpec('Proteina', this.resolveProteinLabel(product.proteinSource));
  ...
  get brandSummary(): string {
    ...
    const audience = this.resolveAudienceLabel(product);
    const parts = [
      `${product.brand.name} participa en este producto dentro del catalogo.`,
      form ? `Se presenta como ${form}.` : '',
      audience ? `Esta orientado a ${audience.toLowerCase()}.` : '',
    ].filter(Boolean);
  ...
  addToCart(): void {
    ...
    this.executeAddToCart(this.product.id, this.qty, 'Producto agregado al carrito.');
  }

  addRelatedToCart(product: Product): void {
    this.executeAddToCart(product.id, 1, `${product.name} agregado al carrito.`);
  }
  ...
  private resolveLeadText(product: StorefrontProduct): string {
    ...
    const parts = [
      `${product.name} es una opcion de ${category} pensada para una compra mas informada.`,
      form ? `Formato: ${form}.` : '',
      protein ? `Proteina principal: ${protein}.` : '',
  ...
  private extractPlainText(value: StorefrontProduct['description']): string {
    ...
    return value.map((node) => this.extractNodeText(node)).join(' ').replace(/\s+/g, ' ').trim();
  }
  ...
  private updatePetRecommendations(): void {
    ...
    this.petRecommendations = this.currentPets.map((pet) => this.buildPetRecommendation(pet, this.product!));
  }
